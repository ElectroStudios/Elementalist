...
public class FireExplosionEntity extends SpellMasterEntity {
    ...
    public FireExplosionEntity(LivingEntity owner, DamageType damageType) {
        super(ModEntities.FIRE_EXPLOSION.get(), owner, damageType);
    ...
    private void explode() {
        this.level.explode(getOwner(), targetPos.x, targetPos.y, targetPos.z, 3, true, Explosion.BlockInteraction.DESTROY);
        this.discard();
    }
    ...
    private void clientExplode() {
        ...
        for (float i = 0; i <= 1; i += 0.02) {
            Vec3 particlePos = Utility.vectorInterp(this.position(), targetPositionClient, i);
            this.level.addParticle(ModParticles.FIRE_EXPLOSION_PARTICLES.get(), particlePos.x, particlePos.y, particlePos.z, 0, 0, 0);
        }
        for (int i = 0; i < 150; i++) {
            ...
            this.level.addParticle(ModParticles.FIRE_FLASH_PARTICLES.get(), randomVector.x,
