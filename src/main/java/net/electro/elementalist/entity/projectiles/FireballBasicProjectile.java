...
public class FireballBasicProjectile extends MasterSpellProjectile {
    ...
    public FireballBasicProjectile(LivingEntity owner, DamageType damageType) {
        super(EntityRegistry.FIREBALL_BASIC.get(), owner, 1, damageType);
    }

    @Override
    protected void projectileTickEffects() {
        this.level().addParticle(ParticleRegistry.FIRE_EXPLOSION_PARTICLES.get(), this.getX(), this.getY(), this.getZ(),
                0, 0, 0);
    }
    ...
    public void explodeClient() {
        for (int i = 0; i < 30; i++) {
            ...
            this.level().addParticle(ParticleRegistry.FIRE_EXPLOSION_PARTICLES.get(), true,
