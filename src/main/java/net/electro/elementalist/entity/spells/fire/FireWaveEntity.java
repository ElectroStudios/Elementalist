...
public class FireWaveEntity extends MasterSpellEntity {
    ...
    public FireWaveEntity(LivingEntity owner, DamageType damageType) {
        super(EntityRegistry.FIRE_WAVE.get(), owner, damageType);
            ...
            if (duration == 1) {
                for (Vec3 explodePos : targetPositions) {
                    for (int i = 0; i < 20; i++) {
                        ...
                        this.level().addParticle(ParticleRegistry.FIRE_FLASH_PARTICLES.get(), particlePos.x, particlePos.y, particlePos.z,
                                0, 0.8f, 0);
                    }
                }
