package net.electro.elementalist.entity.spells.water;
...
public class WaterStreamEntity extends MasterSpellEntity implements IEffectSpell {
    ...
    public WaterStreamEntity(LivingEntity owner, DamageType damageType) {
        super(EntityRegistry.WATER_STREAM.get(), owner, damageType);
    ...
    private void DealDamage() {
        DamageDealer dd = new DamageDealer(this.position(), getOwner(), this, this.ignoredEntities, this.damageType) {
            ...
            public void damageEffects(LivingEntity entity, float effectAmount, Vec3 direction) {
                EffectUtil.addEffect(EffectRegistry.WET.get(), 100, 1, entity);
                MessageRegistry.sendToAllPlayers(new SyncEffectsS2CPacket(SOURCE.getId(), entity.getId()));
    ...
    public void effectClient(LivingEntity target) {
        EffectUtil.addEffect(EffectRegistry.WET.get(), 100, 1, target);
    }
}